...
export class AttributeRollDialog extends FormApplication {
  ...
  async getData(options) {
    ...
    return {
      ...
      damage_roll: this.damage_mod !== null,
  ...
  damage_modifiers_from_target() {
    const target_mods = [
      {
        name: game.i18n.localize("Elemental.Status.Paralyzed"),
        value: -3,
        checked: false,
      },
    ];
    if (game.user.targets.size > 0) {
      const paralyzed = game.user.targets
        .first()
        .actor.effects.getName(
          game.i18n.localize("Elemental.Status.Paralyzed"),
        );
      if (paralyzed) {
        target_mods[0].checked = true;
      }
    }
    return target_mods;
  }
